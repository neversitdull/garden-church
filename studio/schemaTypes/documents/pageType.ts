...
export const pageType = defineType({
  ...
  type: 'document',
  icon: DocumentIcon,
  description: 'Create a new page on your website. For example: About, Contact, Ministries, etc.',
  fields: [
    defineField({
      name: 'title',
      type: 'string',
    ...
    defineField({
      ...
      description: 'Used for SEO. This summary appears in search engine results.',
      rows: 3,
      validation: (Rule) => [
    ...
    defineField({
      ...
      type: 'reference',
      to: [{type: 'page'}],
      description: 'Select a parent page if this is a subpage (e.g. Youth under Ministries).',
      validation: (Rule) =>
        Rule.custom(async (value, context) => {
          ...
          const checkCircular = async (parentId: string, originalId: string): Promise<boolean> => {
            const parent = await client.fetch(`*[_id == $parentId][0]{_id, parent}`, {parentId})
    ...
    defineField({
      ...
      description: 'This image will be used when the page is shared on social media.',
      options: {
        hotspot: true,
      },
    }),
