...
export function PathnameFieldComponent(props: ObjectFieldProps<SlugValue>) {
  ...
  const handleChange = useCallback(
    (value?: string) => {
      const finalValue = value
        ? value.replace(/\/+$/, "").replace(/^\//, "") // clean leading/trailing slashes
        : undefined;
      onChange(
        typeof value === "string"
          ? set({
  ...
  const handleGenerate = useCallback(() => {
    ...
    if (title) {
      ...
      if (newSegments.length > 1) {
        newSegments[newSegments.length - 1] = slugified;
      } else {
        newSegments[0] = slugified;
      }
  ...
  const unlockFolder = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
  ...
  const localizedPathname = `/${value?.current?.replace(/^\//, "") ?? ""}`;
